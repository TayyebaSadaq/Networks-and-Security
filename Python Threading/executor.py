import concurrent.futures# For concurrent execution with ThreadPoolExecutor
import logging
import threading
import time
...
if __name__ == "__main__":
    format = "%(asctime)s: %(message)s" #configure logging with a specfic mesage format and level 
    logging.basicConfig(format=format, level=logging.INFO,
                        datefmt="%H:%M:%S")
    #use a threadpoolexecutor to manage thread execution
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
        #use the executor's map function to run thread_function concurrently for a range of values (0,1,2)
